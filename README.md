-  Hi there! I’m Amruth Ayaan
-  How to reach out to me - amrutayan6@gmail.com



 
Some tools/websites that I liked are:


    - https://6it.dev/blog/page/1  
    - https://github.com/sparkengineering/host-usb-pmod
    - https://www.janestreet.com/puzzles/archive/
    - https://github.com/NXP/eiq-model-zoo/blob/main/tasks/vision/classification/visual-wake-word/README.md
    - https://github.com/Digilent/vivado-boards/tree/master
    - https://formal.org/
    - https://godbolt.org/
    - https://github.com/HewlettPackard/cacti
    - https://github.com/lutzroeder/netron
    - https://github.com/HarisIqbal88/PlotNeuralNet.git
    - https://www.cse.iitd.ac.in/~srsarangi/archbook/software/emuSimpleRisc.zip
    - https://www.cse.iitd.ac.in/~srsarangi/tejasweb/index.html
    - https://www.cse.iitd.ac.in/~srsarangi/archbook/software/interpreter.zip
    - https://cpulator.01xz.net/?sys=arm
    - https://elixir.bootlin.com/linux/v6.2.12/source/kernel
    - https://playground.tensorflow.org/#activation=tanh&batchSize=10&dataset=circle&regDataset=reg-plane&learningRate=0.03&regularizationRate=0&noise=0&networkShape=4,2&seed=0.78113&showTestData=false&discretize=false&percTrainData=50&x=true&y=true&xTimesY=false&xSquared=false&ySquared=false&cosX=false&sinX=false&cosY=false&sinY=false&collectStats=false&problem=classification&initZero=false&hideText=false
    - https://poloclub.github.io/cnn-explainer/
    - Graph tool - Graphviz or Netron 

    Pro tip to myself: find / -type f -name "<filename>" 2>/dev/null 

    Pro tip to myself: Q.E.D. stands for the Latin phrase quod erat demonstrandum, which translates to "that which was to be demonstrated"
    
    Pro tip to myself: Whenever you wanna name any product / tool / whatever just come over here - https://greekgodsandgoddesses.net/goddesses/ 
    
    Pro tip to myself: Whenever you wanna find a good research paper, google scholar is the go to site to begin the search!
    
    Pro tip to myself: wine ~/.wine/drive_c/Program\ Files\ (x86)/LTC/LTspiceIV/scad3.exe
